...
//----------------------------------------
// Constructors
gpcc::EncoderThreadPyramid::EncoderThreadPyramid(PointCloud *ppc, EncoderConfigParams *params, gpcc::Axis axis, uint32_t nParallelism) : acbstream(nParallelism + 1, nullptr)
{
	this->pPC = ppc;
	this->pParams = params;
	this->nBitsAC = 0;
...
gpcc::EncoderThreadPyramid::~EncoderThreadPyramid()
{
	for (auto &bitstream : acbstream)
...
//----------------------------------------
// Public Functions
gpcc::encodeThreadPyramidReturnData gpcc::EncoderThreadPyramid::encodeTopPyramid(uint32_t level, Cabac *cabacIn, uint32_t iStart, uint32_t iEnd,
																																								 gpcc::ImageSparse *fatherSilhouette)
{
	...
	gpcc::encodeThreadPyramidReturnData encodingMetaData;
	...
	if (testDyadicDecompositionMode)
	{
		...
		if (encodeLeft && (level < log2(this->nParallelism) - 1))
		{
			...
			encodeThreadPyramidReturnData leftData = this->encodeTopPyramid(level + 1, cabacDyadic, lStart, lEnd, leftChild);
		...
		if (encodeRight && (level < log2(this->nParallelism) - 1))
		{
			...
			encodeThreadPyramidReturnData rightData = this->encodeTopPyramid(level + 1, cabacDyadic, rStart, rEnd, rightChild);
	...
	if (testSingleProjectionMode)
	{
		...
		// std::cout << "Entering Interval Single Mode Top Pyramid" << std::endl;
		encodeThreadPyramidReturnData singleModeData = this->encodeIntervalSingleModeTopPyramid(cabacSingleMode, iStart, iEnd, fatherSilhouette, 0);
	...
	if (level == 0)
	{
		...
		this->acbstream[0]->writeNumber(length_encoded_param, 16);
		...
		this->acbstream[0]->writeNumber(length_acbstream, 24);

		// Merge the paramBitstream with the final bitstream.
		this->acbstream[0]->merge(paramBitstream);
		this->acbstream[0]->merge(*encodingMetaData.cabacOut->acbstream);
		...
		// std::cout << "LUMA_E(" << 0 << "): " << length_encoded_param << " (" << length_param << ") : " << length_acbstream << std::endl;
	}
...
//----------------------------------------
// Public Functions
gpcc::encodeThreadPyramidReturnData gpcc::EncoderThreadPyramid::encodePointCloud(uint32_t npointcloud, uint32_t level, Cabac *cabacIn, uint32_t iStart,
																																								 uint32_t iEnd, gpcc::ImageSparse *fatherSilhouette)
{
	...
	if (fatherSilhouette->NumberOccupiedPixels() == 0)
	{
		this->acbstream[npointcloud]->writeBit(false);

		delete fatherSilhouette;
		fatherSilhouette = nullptr;

		encodingMetaData.cabacOut->copyCabac(cabacIn);
		encodingMetaData.encodingbitsSize = 1;
	...
	if (testDyadicDecompositionMode)
	{
		...
		if (encodeLeft && (level < (this->nbits - 1)))
		{
			...
			encodeThreadPyramidReturnData leftData = this->encodePointCloud(npointcloud, level + 1, cabacDyadic, lStart, lEnd, leftChild);
			cabacDyadic->copyCabac(leftData.cabacOut);
			leftData.cabacOut->~Cabac();
			nBitsDyadicMode += leftData.encodingbitsSize;
		}
		...
		if (encodeRight && (level < (this->nbits - 1)))
		{
			...
			encodeThreadPyramidReturnData rightData = this->encodePointCloud(npointcloud, level + 1, cabacDyadic, rStart, rEnd, rightChild);
			cabacDyadic->copyCabac(rightData.cabacOut);
			rightData.cabacOut->~Cabac();
			nBitsDyadicMode += rightData.encodingbitsSize;
		}
	...
	if (testSingleProjectionMode)
	{
		...
		encodeThreadPyramidReturnData singleModeData = this->encodeIntervalSingleMode(cabacSingleMode, iStart, iEnd, fatherSilhouette, npointcloud);
		cabacSingleMode->copyCabac(singleModeData.cabacOut);
		singleModeData.cabacOut->~Cabac();
		nBitsSingleMode += singleModeData.encodingbitsSize;
	}
	...
	if (level == log2(this->nParallelism))
	{
		...
		//  - 16 bits for the lenght of the encoded parambitstream.
		//  - 24 bits for the AC payload
		// If I am here, then for sure I have to output something.
		this->acbstream[npointcloud]->writeBit(true);
 ...
 */
void gpcc::EncoderThreadPyramid::writeHeader(uint16_t nBits, uint16_t axis, uint16_t algorithmChoice, uint16_t nParallelism, Bitstream &bstream)
{
...
Bitstream *gpcc::EncoderThreadPyramid::closeBitstream()
{
	...
	this->writeHeader(nBits, axis, EncoderConfigParams::DD_SM_PYRAMID_THREAD, log2((uint16_t)this->nParallelism), *finalBitstream);
...
gpcc::encodeThreadPyramidReturnData gpcc::EncoderThreadPyramid::encodeIntervalSingleMode(Cabac *cabacIn,
																																												 uint32_t iStart, uint32_t iEnd, gpcc::ImageSparse *fatherSilhouette, uint32_t npointcloud)
{
	...
	encodeThreadPyramidReturnData encodingMetaData;
...
gpcc::encodeThreadPyramidReturnData gpcc::EncoderThreadPyramid::encodeIntervalSingleModeTopPyramid(Cabac *cabacIn,
																																																	 uint32_t iStart, uint32_t iEnd, gpcc::ImageSparse *fatherSilhouette, uint32_t npointcloud)
{
	// TODO:
	//  Test if I should add a new table for 3Dcontexts (it seems this is not the case)
	...
	uint32_t numberOfSilhouettesInBlock = 1 << (this->nbits - this->pParams->getNThreads());

	// Make a raster object of this image.
	leftImageRaster = new ImageRaster(this->imSize, this->imSize);
	...
	{
		// Creaters the current silhouette.
		currImageSparse = this->pPC->SilhouetteFromPointCloud(k, k + numberOfSilhouettesInBlock - 1, this->axis);
		currImageRaster = new ImageRaster(currImageSparse, this->imSize, this->imSize, 0);
		...
		if (encodeThisSilhouette)
		{
			...
			// std::cout << "Silhouette(" << k << "," << k + numberOfSilhouettesInBlock - 1 << ") = " << (b2 - b1) << " bits." << std::endl;
#if ENCODER_VERBOSE
			std::cout << "Silhouette(" << k << "," << k << ") = " << (b2 - b1) << " bits." << std::endl;
#endif
		}
...
uint32_t gpcc::EncoderThreadPyramid::getResolution()
{
	return this->nbits;
}

gpcc::Axis gpcc::EncoderThreadPyramid::getAxis()
{
	return this->axis;
}

gpcc::PointCloud *gpcc::EncoderThreadPyramid::getPointCloud()
{
	return this->pPC;
}