...
#define DecoderThread_VERBOSE 0

//----------------------------------------
// Constructors
gpcc::DecoderThread::DecoderThread(Bitstream &pbstream, EncoderConfigParams *params) : nBits(0),
																																											 axis(gpcc::Axis::X),
																																											 acbstream()
{
	this->pParams = params;
...
gpcc::DecoderThread::~DecoderThread()
{
...
void gpcc::DecoderThread::parseHeader(Bitstream &bstream)
{
	...
	this->nParallelism = 1 << (uint16_t)bstream.readNumber(4);
	this->axis = ((iAxis == 0) ? gpcc::Axis::X : ((iAxis == 1) ? gpcc::Axis::Y : gpcc::Axis::Z));

	uint16_t nPointClouds = this->nParallelism;
...
#if DecoderThread_VERBOSE
		std::cout << "Inside DecoderThread" << std::endl;
 ...
 * LUMA
 */
uint32_t gpcc::DecoderThread::getNumberOfPointClouds()
{
	return this->nParallelism;
}

uint16_t gpcc::DecoderThread::getPCResolution()
{
	return this->nBits;
}
/*
 ...
 */
gpcc::PointCloud *gpcc::DecoderThread::decodePointCloud(uint32_t npointcloud)
{
	Cabac localCabac;
	PointCloud *ppc = new PointCloud();
	uint64_t m = 16;
	uint64_t nc2D = 6;
	uint64_t nc3D = 9;
	// std::cout << "LUMA(" << npointcloud << "): " << this->acbstream[npointcloud]->numberOfRemainingBits() << std::endl;
	localCabac.initDecoder(m, nc2D, nc3D, *(this->acbstream[npointcloud]));
	...
	// silhouette_list = SilhouetteDecomposition(this->nBits, this->nParallelism, npointcloud);
	// silhouette_list.createDyadicDecomposition();
	// silhouette_list.printList();
	...
	// 		bool currBit = paramBitstream[npointcloud].readBit();
	...
	// 			{
	// 				// Decodes this image.
	// 				localCabac.decodeImage(*currImage);
	// 			}
	// 			else
	// 			{
	...
	// 					// Decodes the image
	// 					localCabac.decodeImage(*currImage, *maskImage);
	// 				}
	// 				else
	// 				{
	...
	// 						// Decodes the image
	// 						localCabac.decodeImage(*currImage, *maskImage, *leftBrotherImage);
	...
	// #if DecoderThreadThread_VERBOSE
 ...
 */
void gpcc::DecoderThread::decodePointCloud2(uint32_t npointcloud,
																						uint32_t iStart, uint32_t iEnd)
{
	Cabac *decoderThreadCabac = new Cabac();
	uint64_t m = 16;
	uint64_t nc2D = 6;
	uint64_t nc3D = 9;
	// std::cout << "LUMA(" << npointcloud << "): " << this->acbstream[npointcloud]->numberOfRemainingBits() << std::endl;
	...
	{
		decoderThreadCabac->initDecoder(m, nc2D, nc3D, *(this->acbstream[npointcloud]));

		this->imSize = 1 << this->nBits;

		// Calls the recursive decoding function
		this->xDecodePointCloud(npointcloud, decoderThreadCabac, log2(this->nParallelism), iStart, iEnd);
	}
	decoderThreadCabac->~Cabac();
 ...
 */
void gpcc::DecoderThread::xDecodePointCloud(
 ...
 *
 */
void gpcc::DecoderThread::decodeIntervalSingleMode(uint32_t npointcloud, Cabac *cabacIn, uint32_t iStart, uint32_t iEnd, gpcc::ImageSparse *fatherSilhouette)
{
	// TODO:
 ...
 */
void gpcc::DecoderThread::deducePixels(gpcc::ImageRaster &image, gpcc::ImageSparse &fatherImage, gpcc::ImageRaster &leftImage)
{
...
gpcc::PointCloud *gpcc::DecoderThread::getPointCloud(uint32_t npointcloud)
{
