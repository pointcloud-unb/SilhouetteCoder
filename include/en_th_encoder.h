...
namespace gpcc
{
    ...
    private:
        ...
        gpcc::encodeThreadReturnData encodeIntervalSingleMode(Cabac *cabacIn, uint32_t iStart, uint32_t iEnd, gpcc::ImageSparse *fatherSilhouette, uint32_t npointcloud);
        void writeHeader(uint16_t nBits, uint16_t axis, uint16_t algorithmChoice, uint16_t nParallelism, Bitstream &bstream);

    public:
        EncoderThread(PointCloud *ppc, EncoderConfigParams *params, gpcc::Axis axis, uint32_t nParallelism);
        ~EncoderThread();
        encodeThreadReturnData encodePointCloud(uint32_t npointcloud, uint32_t level, Cabac *cabacIn,
